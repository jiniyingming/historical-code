...
class DingCallbackCrypto
{
	...
	public function getDecryptMsg($signature, $timeStamp = null, $nonce, $encrypt)
	{
		if (strlen($this->m_encodingAesKey) != 43) {
			...
			throw new \RuntimeException('IllegalAesKey', ErrorCode::$IllegalAesKey);
		}
		...
		if ($ret != 0) {
			//return $ret;
			// return ['ErrorCode'=>$ret, 'data' => ''];
			throw new \RuntimeException('ComputeSignatureError', ErrorCode::$ComputeSignatureError);
		}
		$verifySignature = $array[1];
		if ($verifySignature != $signature) {
			throw new \RuntimeException('ValidateSignatureError', ErrorCode::$ValidateSignatureError);
		}
		...
		if ($result[0] != 0) {
			...
			throw new \RuntimeException('DecryptAESError', ErrorCode::$DecryptAESError);
		}
		$decryptMsg = $result[1];
